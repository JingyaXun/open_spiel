...
#   with open("dynamics.txt", "a") as f:
#     f.write(str(state * (fitness - avg_fitness))+"\n")

  return state * (fitness - avg_fitness)

def replicator_beta(state, fitness, beta=5):
  """Continuous-time replicator dynamics.
  ...
  Returns:
    Time derivative of the population state.
  """
  avg_fitness_beta = None
  dynamics = None

  if beta == 0:
    avg_fitness_beta = np.mean(fitness, axis = 0)
    dynamics = fitness - avg_fitness_beta
  else:
    state_beta = np.power(state, beta)
    avg_fitness_beta = state_beta.dot(fitness)
    dynamics = (fitness - avg_fitness_beta / np.sum(state_beta)) * state_beta

#   with open("dynamics_beta.txt", "a") as f:
#     f.write(str((fitness - avg_fitness_beta / np.sum(state_beta)) * state_beta)+"\n")
